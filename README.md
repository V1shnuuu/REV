# Revive
A Hybrid Edge-AI Cardiac Emergency Response System that coaches bystanders through CPR in the critical minutes before EMS arrives.

**Works Offline** • **Edge AI** • **Flutter** • **Health Connect**
...
Revive is built around being honest about both of those, and using the LLM only where it earns its place, and falling back to a rock-solid offline core.
...
```mermaid
flowchart TD
    Start([Emergency Started]) --> Check{Connectivity Available?}
    Check -- Yes --> Triage[Voice Triage Quick Check]
    Triage --> LLM[Gemma Classifies Situation]
    LLM -- Cardiac Arrest --> CPR[Start Standard CPR Protocol]
    LLM -- Choking --> Choking[Choking Protocol]
    LLM -- Drowning --> Drowning[Drowning Protocol]
    
    Check -- No (Offline) --> CPR
    
    subgraph Core [On-Device Core System - Never Dependent on Network]
        CPR --> Motion[Accelerometer Tracking]
        Motion --> Rhythm[Rhythm & Rate Judgment 100-120 BPM]
        Rhythm --> Feedback[Audio Metronome & Haptics]
        
        Voice[On-Device Voice Recognition] -- "Emergency" keyword --> Dial[Trigger Emergency Dial]
    end
    
    Voice -- Q&A during CPR --> Check
```
...
- **Bystander triage.** Before CPR starts, an optional "Quick Check" step lets the bystander describe what's happening in their own words. Gemma classifies it into cardiac arrest, choking, or drowning — three situations with three different first actions — and branches the guidance accordingly.
...
If the AI is unreachable at any point, the rescuer keeps getting compression coaching, rhythm feedback, and step-by-step instructions. That's what "hybrid" means here: AI is additive, never load-bearing.
...
```mermaid
sequenceDiagram
    participant App as Revive App
    participant OS as Android Health Connect
    participant EMS as EMS / Hospital Staff
    
    App->>App: CPR Session Concludes
    App->>App: Compile Incident Record<br/>(Compression count, Duration, Mode)
    App->>OS: Check permissions & availability
    alt Permission Granted
        App->>OS: Write Timestamped Session Record
    else Permission Denied / Not Installed
        App->>App: Fail silently (no interruption)
    end
    
    EMS->>OS: Access incident history upon arrival
```
...
Revive is built on a token-driven design system rather than ad-hoc styling — semantic colour tokens, a typographic scale, spacing, shape, motion and haptic vocabularies, all in `lib/theme/`, with no raw hex values anywhere else in the app. See **[DESIGN_SYSTEM.md](DESIGN_SYSTEM.md)**.

- **Red is reserved for "stop and call for help."** Out-of-range compression rate is a normal, transient state during correct CPR, so rhythm feedback uses amber. Using alarm-red for it would risk alarm fatigue on the one signal that must never be ignored.
- **Never colour alone.** Every state carries colour *and* icon *and* text *and* motion, so any single channel conveys the instruction on its own.

## Local Setup & Installation

Follow these instructions to run the full application locally:

### 1. AI backend (optional — core CPR coaching works without this)
...
│   ├── services/       # Core services (Audio, Motion, Ollama)
│   ├── theme/          # Token-driven design system (Colors, Typography)
│   └── main.dart       # Application entry point
├── test/               # Flutter test suite (Unit, Widget, Accessibility)
├── tool/               # Helper scripts (e.g., contrast_check.py)
├── pubspec.yaml        # Flutter project dependencies
├── README.md           # Repository documentation
├── DESIGN_SYSTEM.md    # Design tokens and UI documentation
└── QA_CHECKLIST.md     # Manual testing and QA checklist
```
...
Device-dependent verification that automation cannot cover is tracked in **[QA_CHECKLIST.md](QA_CHECKLIST.md)**, along with the current known gaps.
